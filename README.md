...
|----------|-------------|---------|
| `GPU_API_URL` | HTTPS URL of the GPU API server | `https://glorfindel:9443` |
...
```bash
scp vision/gpu_server.py vision/setup_gpu_server.sh vision/requirements-server.txt flame@your-gpu-host:~/ai-studio/vision/
ssh flame@your-gpu-host "cd ~/ai-studio/vision && bash setup_gpu_server.sh"
```
...
```json
{
  "mcpServers": {
    "maya-mcp": {
      ...
      "env": {
        "GPU_API_URL": "https://glorfindel:9443",
...
GPU_API_URL=https://glorfindel:9443  # GPU API server URL
