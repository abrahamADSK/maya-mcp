...
```bash
git clone https://github.com/YOUR_USERNAME/hunyuan3d-maya-pipeline.git
cd hunyuan3d-maya-pipeline
```
...
| `PROJECT_DIR` | Absolute path to this repo on your Mac | `/Users/you/projects/hunyuan3d-maya-pipeline` |
...
import os
os.environ['PROJECT_DIR'] = '/path/to/hunyuan3d-maya-pipeline'
...
exec(open('/path/to/hunyuan3d-maya-pipeline/vision/pipeline_runner.py').read())
...
```python
# In Maya Script Editor → Python tab
exec(open('/path/to/hunyuan3d-maya-pipeline/vision/maya_import_hires.py').read())
```

Then apply position correction and smooth subdivision:

```python
exec(open('/path/to/hunyuan3d-maya-pipeline/vision/maya_fix_position_smooth.py').read())
```
...
```json
{
  "mcpServers": {
    "maya": {
      "command": "/path/to/hunyuan3d-maya-pipeline/core/.venv/bin/python",
      "args": ["/path/to/hunyuan3d-maya-pipeline/core/server.py"]
    }
  }
}
```
...
# Local project
PROJECT_DIR=/path/to/hunyuan3d-maya-pipeline
