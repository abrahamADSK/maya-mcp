...
| `render_still` | Single-frame **Arnold** ray-traced still to a PNG at the exact out_path (long op). This is what "a still" means — a real render, not the Viewport 2.0 grab that maya_viewport_capture does. Maya only exports a `.ass`; **kick renders it out of process**, which is what makes the review view transform apply (dumping Maya's Render View writes a scene-linear file) and means no Render View window is opened and the render can never hang Maya's main thread. Params: out_path (required), camera, frame, width, height, aa_samples, view_transform, timeout. |
