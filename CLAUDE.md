...
MAYA_APP=                    # OPTIONAL pin for WHICH Maya `launch` opens: absolute .app path or a
                             # selector matching exactly one install (e.g. "2027"). Empty (normal)
                             # = open the NEWEST install and report it, warning when others exist.
                             # Never `open -a Maya`, which lets macOS pick blindly (Chat 94).
                             # Pipeline authority is the SG-Desktop default → use fpt_launch_app.
...
| `render_still` | Single-frame **Arnold** ray-traced still → PNG at the exact out_path (long op). This is what "a still" means — a real render, not the VP2.0 grab of `maya_viewport_capture`. Maya only exports a `.ass`; **kick renders it out of process** (the `catcher-passes` pattern). That split is what makes the review view transform apply — dumping Maya's Render View writes a **scene-linear** file and ignores every colour setting (measured 127 vs 188 on a flat 0.5 patch, Chat 94) — and it means no Render View window is opened and the render can never hang Maya's main thread. Params: out_path (required), camera, frame, width, height, aa_samples, view_transform, timeout. |
