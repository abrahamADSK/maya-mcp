...
def build_system_prompt(available_servers: dict) -> str:
    ...
    if "maya-mcp" in available_servers:
        parts.append(
            ...
            "     · still / \"render a still\" / \"un still\" / \"frame renderizado\" / \"render en "
            "Arnold\" → `maya_session action=render_still` (a REAL Arnold ray-traced frame → PNG at "
            "out_path, resolve via fpt tk_resolve_path into the review area). This is what a "
            "\"still\" means — NOT `maya_viewport_capture`, which is only a fast VP2.0 screenshot "
            "(use that solely when the user explicitly wants a quick viewport grab / pantallazo).\n"
            "   NEVER hand-build a turntable/still playblast or an arnoldRender loop with "
            "execute_python — improvising it yields an empty/wrong frame and can hang Maya's main "
            "thread (a playblast over a live Arnold IPR panel saturates it); the deterministic "
            "review_turntable / render_still actions already do it right and never touch the "
            "user's viewport."
        )
    ...
    if "fpt-mcp" in available_servers:
        parts.append(
            ...
            "revisión\". (a) Resolve the .mov path with `tk_resolve_path` (template "
            "`movie_asset_publish`, **name = the TASK name**, e.g. `Model`) so the FILE is "
            "`{Asset}_{Task}_v###.mov` (e.g. `DJ_Model_v001.mov`) — NEVER `name='turntable'`. "
            "(b) `review_turntable` to that path. (c) `sg_create` type=Version with code = its "
            "returned `version_code` EXACTLY (same {Asset}_{Task}, e.g. `DJ_Model`) — NEVER the "
            ".mov filename; link entity + task, set sg_path_to_movie to the .mov, then "
            "`sg_upload` the .mov to sg_uploaded_movie (+ sg_path_to_frames for an exr sequence).\n"
            "       A STILL for review is the SAME flow with `render_still` instead of the "
            "turntable — the important part of \"a still for review\" is the Version, NOT a loose "
            "PNG on disk: (a) resolve a PNG path with `tk_resolve_path` in the review area; (b) "
            "`maya_session action=render_still` to it (Arnold; returns `version_code` {Asset}_{Task}); "
            "(c) `sg_create` type=Version with code = that `version_code`, link entity + task, set "
            "sg_path_to_frames to the PNG, then `sg_upload` the PNG to sg_uploaded_movie so it is "
            "reviewable media (a still is valid review media). ALWAYS create+upload the Version — "
            "rendering the PNG alone does NOT put it in review.\n"
