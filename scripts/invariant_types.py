...
def _extract_items(source: dict) -> set[str]:
    ...
    if kind == "ast_decorator_kwarg":
        # For decorators like @mcp.tool(name="maya_xxx") where the public name
        # is passed as a keyword argument, extract that kwarg's value for every
        # decorated function. When the kwarg is absent, fall back to the
        # Python function name (the decorator's implicit default). Useful for
        # projects where the Python symbol name and the exposed tool name
        # differ (e.g. `search_maya_docs_tool` → `search_maya_docs`).
        tree = ast.parse(_read(source["file"]))
        decorators = source.get("decorators") or [source["decorator"]]
        kwarg = source["kwarg"]
        items: set[str] = set()
